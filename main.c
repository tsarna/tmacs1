...
char *argv[];
{
	...
	static int	startflag;	/* startup executed flag */
	static BUFFER *firstbp = NULL;		/* ptr to first buffer in cmd line */
	int basec;			/* c stripped of meta character */
	static int viewflag;			/* are we starting in view mode? */
	...
	if (startflag == FALSE) {
		...
		if (!firstfile && strcmp((char *)pathname[0], firstbp->b_bname) == 0) {
			c = firstbp->b_bname[0];
			firstbp->b_bname[0] = '[';
			startup("");
			firstbp->b_bname[0] = c;
...
execute(c, f, n)
{
	 ...
	 * and we are not read-only, perform word wrap.
		...
		if (c == '}' && (curbp->b_mode & (MDPYMOD|MDCMOD)) != 0)
	        	status = insbrace(n, c);
